...
- 管理测试知识库（日志、缺陷单、技术文档），支持上传、检索、增量索引；非 UTF-8 编码（GB2312 等）自动检测回退，超大文本截断降级索引
- 基于 RAG 的智能问答：召回-重排两阶段检索（向量召回 top10 → LLM 精排取 top3），显示引用来源和相关度分数；查询结果缓存（memory/Redis 双后端，知识库变更自动失效）
...
- 安全：认证 fail-closed、SSRF/路径穿越防护、PII 脱敏（手机号/邮箱/身份证/银行卡不出现在 LLM 上下文）
